...
async function resolveAndScrapeShopFromUrl(rawUrl: string): Promise<{ type: string; data?: any; message: string }> {
 ...
 console.log('[Background] 商品解析成功 productID=' + product.productID + ' shopID=' + (product.shopID || '-'));
 ...
 console.error('[Background] 商品采集失败:', err);
...
async function scanShop(shop: Shop, workerUrl: string): Promise<number> {
 ...
 console.log(`[Background] [${i + 1}/${productLinks.length}] 商品解析成功 productID=${product.productID} name=${product.name}`);
 ...
 console.log(`[Background] 评论入库成功 productID=${product.productID} reviews=${reviews.length}`);
 }
 } catch (revErr) {
...
async function extractShopLinksUsingScripting(tabId: number, shopSlug: string): Promise<string[]> {
 ...
 const tabEls = Array.from(document.querySelectorAll('a, button, [role="tab"], div[data-testid*="tab"]'));
 ...
 console.log(`[Background] Scripting 提取完成，候选链接数: ${(results && results[0]?.result || []).length}`);
